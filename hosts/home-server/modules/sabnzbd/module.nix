# taken from https://github.com/NixOS/nixpkgs/pull/416165
{
  config,
  lib,
  pkgs,
  ...
  configObjIni = {}: let
    ...
    mkSection = (
      depth: attrs: let
        ...
        mkSectionHeading = name: "${sectionHeadingLeft}${lib.escape ["[" "]"] name}${sectionHeadingRight}";
  ...
  publicSettingsIni =
    ...
    then cfg.configFile
    else (configObjIni {}).generate "public-settings.ini" allSettings;
          ...
          options = {
            misc = {
              ...
              email_endjob = mkOption {
                ...
                '';
                default =
                  if cfg.settings.misc.email_server != ""
                  then "on error"
                  else "never";
